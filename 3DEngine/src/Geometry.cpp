...
Geometry::MeshData Geometry::CreateBox(float width, float height, float depth, uint32_t numSubdivisions)
{
	...
	//Cap on the number of subdivisions
...
Geometry::MeshData Geometry::CreateSphere(float radius, uint32_t sliceCount, uint32_t stackCount)
{
	...
	for (uint32_t i = 1; i <= stackCount - 1; ++i)
	{
		...
		for (uint32_t j = 0; j <= sliceCount; ++j)
		{
			...
			v.texCord.x = theta / DirectX::XM_2PI;
			v.texCord.y = phi / DirectX::XM_PI;
...
Geometry::MeshData Geometry::CreateCylinder(float bottomRadius, float topRadius, float height, uint32_t sliceCount,
	uint32_t stackCount)
{
	MeshData meshData;
...
void Geometry::BuildCylinderTopCap(float bottomRadius, float topRadius, float height, uint32_t sliceCount,
...
void Geometry::BuildCylinderBottomCap(float bottomRadius, float topRadius, float height, uint32_t sliceCount,
	uint32_t stackCount, MeshData& meshData)
{
	uint32_t baseIndex = (uint32_t)meshData.vertices.size();
	float y = -0.5f * height;
